...
public static class CreateDelayTimer
{
    ...
    public static Timer RunIndefiniteDelayedInterval(this TimerThread thread, Timer.TimerCallback callback, ITimerData data, TimeSpan delay, TimeSpan interval) =>
        thread.DelayCall(callback, data, delay, interval, 0);

    public static Timer RunInterval(this TimerThread thread, Timer.TimerCallback callback, ITimerData data, TimeSpan delay, TimeSpan interval, int count) =>
        thread.DelayCall(callback, data, delay, interval, count);

    private static Timer DelayCall(this TimerThread thread, Timer.TimerCallback callback, ITimerData data, TimeSpan delay, TimeSpan interval, int count)
    {
